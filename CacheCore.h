...
enum    ReplacementPolicy  {LRU,RANDOM,MRU};
...
public:
    ...
    //virtual bool findInvalidLine();
    bool findInvalidLine(){
        return false;
    }
...
class CacheAssoc : public CacheGeneric<State, Addr_t, Energy> {
    ...
    using CacheGeneric<State, Addr_t, Energy>::sets;
...
public:
    ...
    bool findInvalidLine(){
        ...
        for(uint32_t i = 0; i < numLines; i++) {
            lineChosen = content[i];
            if(!lineChosen->isValid())
            {
                return true;
            }
        }
