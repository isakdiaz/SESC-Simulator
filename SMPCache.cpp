...
SMPCache::SMPCache(SMemorySystem *dms, const char *section, const char *name)
    ...
    , compMiss("%s:compMiss", name)   //ADDED
...
uint32_t currentTag = calcTag(addr);


//Run once

int32_t s = SescConf->getInt("DMemory", "size");
int32_t b = SescConf->getInt("DMemory", "bsize");
int32_t numberOfLines = (s/b);


//MSG("number of lines is %d", numberOfLines);
    ...
    if (l && l->canBeRead()) {
        ...
        return;        //All valid l's go through this return and don't register as a readMiss
    }
...
/*Tag Checks******************************************************************************
    if(l && !l->canBeRead()) {
    MSG("11111111111111111111111111111111111111111");
    }

if(cache->findLine(addr)){
    MSG("22222222222222222222222222222222222222");
}

if(cache->readLine(addr)){
    MSG("333333333333333333333333");
}

if(l){
    MSG("55555555555555555555555");
}
*/
...
//If addr is not in vector, then it is a compulsory miss
   
 if (cmap[currentTag] == true && !l){        //If line exists and line is valid then coherence miss
        coheMiss.inc();
    //MSG("Coherence Miss");
    cmap[currentTag] = false;

    }
    else if ( !(std::find(addressList.begin(), addressList.end(), currentTag) != addressList.end()) ) {  

    addressList.push_back(currentTag);               //Adds new tags to vector history
    compMiss.inc();

    //MSG("COMP Write @ %x and %x", addr, currentTag);
  
  
    }else{
        replMiss.inc();             //First time Accessed l is always false
    //MSG("Repl");
...
#if 0
//If addr is not in vector, then it is a compulsory miss
    if ( !(std::find(addressList.begin(), addressList.end(), currentTag) != addressList.end()) ) {  
    ...
    }else if(cmap[currentTag] == true && !l){        //If line exists and line is valid then coherence miss
        coheMiss.inc();
    //MSG("Coherence Miss");
    cmap[currentTag] = false;
